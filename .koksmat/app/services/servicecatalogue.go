...
import (
    ...
    "github.com/magicbutton/magic-meetings/services/models/servicecataloguemodel"
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        ...
        ServiceResponseError(req, fmt.Sprintf("Error calling ServicecatalogueRead: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ServicecatalogueCreate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ServicecatalogueUpdate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    return
            ...
            if (err != nil) {
                log.Println("Error", err)
                ServiceResponseError(req, fmt.Sprintf("Error calling ServicecatalogueDelete: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ServicecatalogueSearch: %s", err))
