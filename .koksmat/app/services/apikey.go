...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        ...
        ServiceResponseError(req, fmt.Sprintf("Error calling ApikeyRead: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ApikeyCreate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ApikeyUpdate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    return
            ...
            if (err != nil) {
                log.Println("Error", err)
                ServiceResponseError(req, fmt.Sprintf("Error calling ApikeyDelete: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ApikeySearch: %s", err))
