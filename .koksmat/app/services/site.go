...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        ...
        ServiceResponseError(req, fmt.Sprintf("Error calling SiteRead: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling SiteCreate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling SiteUpdate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    return
            ...
            if (err != nil) {
                log.Println("Error", err)
                ServiceResponseError(req, fmt.Sprintf("Error calling SiteDelete: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling SiteSearch: %s", err))
